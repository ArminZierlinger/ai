...
def check_module_versions() -> None:
    ...
    scipy_check = '(\u2713)' if Version(scipy.__version__) >= Version('1.5.0') else '(\u2717)'
...
def load_penguins() -> pd.DataFrame:
    ...
    """
    # load the dataset from https://github.com/mcnakhaee/palmerpenguins/blob/master/palmerpenguins/data/penguins.csv

    penguins_data = pd.read_csv("https://raw.githubusercontent.com/mcnakhaee/palmerpenguins/master/palmerpenguins/data/penguins.csv")
    penguins_data = penguins_data.dropna()
    filtered_data = penguins_data[["bill_length_mm", "bill_depth_mm", "flipper_length_mm", "body_mass_g", "species"]]
...
def load_toy_data() -> pd.DataFrame:
    ...
    [1] Toy data set for testing purposes.
...
def load_mnist() -> pd.DataFrame:
    ...
    data = mnist_data["data"].astype(np.float32)
...
def load_cifar10() -> pd.DataFrame:
    ...
    """
    cifar10_data = datasets.fetch_openml(name=r'CIFAR_10', as_frame=True, parser="auto")
    data = cifar10_data["data"].astype(np.float32)
    data["item_type"] = cifar10_data["target"].astype(int)
    return data
...
def load_electricity() -> pd.DataFrame:
    ...
    """
    electricity_data = datasets.fetch_openml(name="electricity", as_frame=True, parser="auto")
    data = electricity_data["data"].astype(np.float32)
    data["day"] = data["day"].astype(int)
    data["demand"] = electricity_data["target"]
    return data

def load_glass_identification() -> pd.DataFrame:
    """
    Load glass identification data set [1].

    [1] B. German. Glass identification data set. UCI Machine Learning Repository, 1987.

    :return: glass identification data set
    """
...
def standardize_data(data: pd.DataFrame) -> pd.DataFrame:
    """
    Scale specified data set using standard scaling.
...
def plot_dendrogram(linked_clusters, figsize=None):
    '''
    Plot a dendrogram.

    :param linked_clusters: linkage matrix
    :return: None
    '''
    if figsize:
